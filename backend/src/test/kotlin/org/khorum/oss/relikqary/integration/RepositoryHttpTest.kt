...
class RepositoryHttpTest {
    ...
    fun `publishes and resolves a file byte-for-byte`() {
        ...
        val path = "/releases/com/example/widget/3.0.0/widget-3.0.0.jar"
    ...
    fun `returns 404 for an unpublished coordinate`() {
        assertEquals(404, get("/releases/com/example/widget/9.9.9/widget-9.9.9.jar").statusCode())
    }

    @Test
    fun `rejects re-publishing an existing release with 409`() {
        val path = "/releases/com/example/locked/1.0.0/locked-1.0.0.jar"
    ...
    fun `allows overwriting an existing snapshot`() {
        val path = "/snapshots/com/example/snap/1.0.0-SNAPSHOT/snap-1.0.0-SNAPSHOT.jar"
    ...
    fun `rejects path traversal attempts`() {
        ...
        val request = HttpRequest.newBuilder(URI.create("${base()}/releases/com/example/%2e%2e%2f%2e%2e%2fescape.jar")).GET().build()
