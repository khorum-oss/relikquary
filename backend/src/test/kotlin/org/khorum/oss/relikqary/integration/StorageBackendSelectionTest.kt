...
/**
 * Backend selection (FR-002): `relikqary.storage.backend=s3` activates the S3 backend. (The default
...
@SpringBootTest(
    properties = [
        "relikqary.security.enabled=false",
        "relikqary.storage.backend=s3",
        "relikqary.storage.s3.endpoint=http://127.0.0.1:1",
        "relikqary.storage.s3.bucket=relikqary",
        "relikqary.storage.s3.access-key=test",
        "relikqary.storage.s3.secret-key=test",
    ],
)
