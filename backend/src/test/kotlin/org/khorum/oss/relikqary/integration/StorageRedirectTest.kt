...
import org.khorum.oss.relikqary.RelikqaryApplication
 ...
 * different `relikqary.storage.filesystem.root` persists artifacts to the new location, with no code
...
class StorageRedirectTest {
    ...
    private fun boot(root: Path): org.springframework.context.ConfigurableApplicationContext =
        SpringApplication.run(
            RelikqaryApplication::class.java,
            "--server.port=0",
            "--relikqary.storage.filesystem.root=$root",
        )
