...
the spec/clarifications). All paths under `frontend/` unless noted. Frontend-focused, with one small
necessary backend change: the 401 entry point omits the `WWW-Authenticate` challenge for browser XHR
(requests carrying `X-Requested-With`) so the browser shows our login form instead of its native
auth dialog — Maven/Gradle wire behaviour is unchanged.

## Phase 1: Setup

- [X] T001 Broaden the dev proxy in `frontend/vite.config.ts` to forward any non-SvelteKit path (exclude
...
- [X] T002 Add `frontend/src/lib/auth.svelte.ts`: a Svelte 5 rune session store backed by `sessionStorage`
  (key `relikquary.auth`) with `login(user, pass)`, `logout()`, `current()`, and `authHeader()`; hydrate
  on load, persist on login, clear on logout.
- [X] T003 Update `frontend/src/lib/api.ts`: throw `ApiError extends Error { status }` on non-OK; attach
...
- [X] T004 [P] [US1] Add `frontend/src/lib/components/KindBadge.svelte` and `RepositoryRow.svelte`
  (name + kind badge; `data-testid="repo-row"`/`"repo-kind"`).
- [X] T005 [US1] Update `frontend/src/routes/+page.svelte` to render the repositories with `RepositoryRow`
  showing each `kind`.
- [X] T006 [US1] Update `frontend/src/routes/r/[repo]/[...path]/+page.svelte` to look up the current repo's
...
- [X] T007 [P] [US2] Add `frontend/src/lib/components/LoginForm.svelte` (username/password + submit,
...
- [X] T008 [US2] Update `frontend/src/routes/+layout.svelte`: header shows "Log in" when anonymous or the
  username + "Log out" when authenticated (`data-testid="login-button"`/`"logout-button"`/`"current-user"`);
  wire to the auth store + `LoginForm`.
- [X] T009 [US2] Update the browse page (`r/[repo]/[...path]/+page.svelte`) to handle `ApiError`: `401`
...
- [X] T010 [P] [US3] Add `frontend/src/lib/components/UploadForm.svelte`: file picker + confirmable target
...
- [X] T011 [US3] Integrate upload into the browse page for **hosted** repos only: call `upload(...)`, on
...
- [X] T012 [US4] Add Storybook for SvelteKit: install `@storybook/sveltekit` + the Svelte-CSF addon as
...
- [X] T013 [P] [US4] Add `*.stories.svelte` next to each component (`KindBadge`, `RepositoryRow`,
  ...
  invalid/uploading).
- [X] T014 [US4] Add a `build-storybook` step to the frontend job in `.github/workflows/ci.yml`.

**Checkpoint**: the component catalog builds and renders.

## Phase 7: Polish, e2e & verify

- [X] T015 [P] Update `frontend/scripts/e2e.sh` to run the backend **auth-enabled** with a publisher user,
...
- [X] T016 [P] Revise `frontend/tests/browse.spec.ts` to the anonymous flow: browse and download an open
  repo (remove the delete step, which now needs auth).
- [X] T017 Add `frontend/tests/authz.spec.ts`: anonymous private browse prompts login; log in → browse the
  private repo → upload a file → see it listed → delete it; assert a `403`/forbidden message for a
  non-permitted action.
- [X] T018 Run `npm run check`, `npm run build`, `npm run build-storybook`, and `bash scripts/e2e.sh` —
