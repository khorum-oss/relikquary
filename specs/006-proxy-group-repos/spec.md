...
- **FR-005**: A proxy MUST reflect the upstream's `maven-metadata.xml` rather than a permanently cached
  copy, so versions published upstream after first contact remain resolvable.
